...
public class ElementoPpDAO {
    ...
    //Método que permite consultar un solo EPP por nombre
    public ArrayList consultarElementoPp(String sql) throws SQLException {
        con = cn.getConnection();
        ps = con.prepareStatement(sql);
        rs = ps.executeQuery();
        ArrayList listElemento = new ArrayList();
        while (rs.next()) {
            epp.setIdCodigoEpp(rs.getInt("idCodigoEpp"));
            epp.setIdCedula(rs.getString("idCedula"));
            epp.setNombreEpp(rs.getString("nombre"));
            listElemento.add(new ElementoPp(epp.getIdCodigoEpp(), epp.getIdCedula(), epp.getNombreEpp()));
        }
        return listElemento; 
    }
